...
def user_activity_report():
    ...
    for activity in activities:
        ...
        user_stats[user_id]['actions'][action] = user_stats[user_id]['actions'].get(action, 0) + 1
...
def dashboard_stats():
    ...
    if current_user.role == UserRole.ADMIN:
        ...
        total_assets = Asset.query.filter_by(department_id=current_user.department_id).count()
        total_departments = 1
        total_users = User.query.filter_by(department_id=current_user.department_id).count()
        
        assets_by_status = db.session.query(
            Asset.status, func.count(Asset.id)
        ).filter_by(department_id=current_user.department_id).group_by(Asset.status).all()
    ...
    return jsonify({
        ...
        'assets_by_status': {status.value: count for status, count in assets_by_status},
