...
class TokenManager:
    ...
    def create_tokens(self, user_id, username, role, ip_address=None, user_agent=None):
        ...
        jwt_access_expires = (
            self.config.get("JWT_ACCESS_TOKEN_EXPIRES")
            if isinstance(self.config, dict)
            else self.config.JWT_ACCESS_TOKEN_EXPIRES
        )
        jwt_refresh_expires = (
            self.config.get("JWT_REFRESH_TOKEN_EXPIRES")
            if isinstance(self.config, dict)
    ...
    def refresh_access_token(self, refresh_jti, ip_address=None, user_agent=None):
        ...
        access_token = create_access_token(
            identity=refresh_data["username"],
            additional_claims={
                "jti": new_access_jti,
